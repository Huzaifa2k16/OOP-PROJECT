    ...
    public int Age
    {
        ...
        set
        {
            if (value >= 20)
                ...
                throw new Exception("Customer age must be 20 or above for registration.");
        }
    }
...
class Customer : Person, IPayment
{
    ...
    public Customer(int id, string name, int age, string cnic)
        : base(name, age)
    {
        CustomerID = id;
        CNIC = cnic;
    ...
    public static int GenerateCustomerID(int age, string cnic, int roomCategory)
    {
        string lastFourDigits = cnic.Substring(cnic.Length - 4);
        int cnicPart = Convert.ToInt32(lastFourDigits);

        int generatedID = (age * 1000) + cnicPart + (roomCategory * 100);
        return generatedID;
    }
...
class Booking
{
    ...
    {
        Console.WriteLine(Customer.Name + " booked Room " + Room.RoomNumber);
    }
    ...
    public void BookRoom(int days)
    {
        ...
        Console.WriteLine("\n===== BOOKING DETAILS =====");
        Console.WriteLine(Customer.Name + " booked " + Room.RoomType);
...
class Hotel
{
    ...
    public const int MaxRooms = 100; // Capacity limit
    ...
    public bool IsRoomAlreadyBooked(int roomNumber)
    {
        return rooms.Exists(r => r.RoomNumber == roomNumber);
    }

    // Composition with Max Capacity & Duplicate Validations
    public void AddRoom(Room room)
    {
        if (TotalRooms >= MaxRooms)
        {
            throw new Exception("Hotel capacity full! Cannot add more than 100 rooms.");
        }

        if (IsRoomAlreadyBooked(room.RoomNumber))
        {
            throw new Exception($"Room {room.RoomNumber} is already booked!");
        }
